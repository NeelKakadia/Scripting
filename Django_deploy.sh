...
code_clone() {
    ...
    if [ -d "Notes_App_Django" ]; then
        echo "The code directory already exists. Skipping clone."
    else
        git clone https://github.com/NeelKakadia/Notes_App_Django.git || {
...
install_requirements() {
    ...
    brew update && brew install docker nginx docker-compose || {
...
deploy() {
    ...
    cd ./Notes_App_Django/ && docker build -t notes-app . && docker-compose up -d || {
...
# Deploy the app
if ! deploy; then
    echo "Deployment failed. Mailing the admin... $(pwd)"
